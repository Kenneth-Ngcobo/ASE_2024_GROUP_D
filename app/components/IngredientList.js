...
export default function IngDisplay({ selectedIngs = [], onIngsChange = () => {} }) {
  ...
  // Effect to update the URL when selected ingredients change
  useEffect(() => {
    const selectedIngredients = searchParams.get('ingredients')?.split(',') || [];
    
    // Only update the URL if the selected ingredients have changed
    if (JSON.stringify(selectedIngs) !== JSON.stringify(selectedIngredients)) {
      const params = new URLSearchParams(searchParams);
      params.set('ingredients', selectedIngs.join(','));
  ...
  // Handle ingredient selection or deselection
  const handleIngSelect = (ing) => {
    if (!selectedIngs.includes(ing)) {
      onIngsChange([...selectedIngs, ing]);
    } else {
