...
const RecordVoice = () => {
  const [instructions, setInstructions] = useState([]); // Stores the fetched instructions
  ...
  const [error, setError] = useState(""); // To display error message if fetching fails

  // Fetch instructions from an API on component mount
  useEffect(() => {
    const fetchInstructions = async () => {
      try {
        const response = await fetch("/api/instructions"); // API endpoint to fetch instructions
  ...
  // Display loading or error state until instructions are fetched
  if (error) {
    return <p style={{ textAlign: "center" }}>Error: {error}</p>;
  }

  if (instructions.length === 0) {
    return <p style={{ textAlign: "center" }}>Loading instructions...</p>;
  }
